...
class TestSparseTensorRoundTrip : public ::testing::Test, public IpcTestFixture {
 public:
  ...
  void CheckSparseCSFTensorRoundTrip(const SparseCSFTensor& sparse_tensor) {
    ...
    int64_t ndim = sparse_index.axis_order().size();
    ...
    for (int i = 0; i < ndim - 1; ++i) {
      out_indptr_length += resulted_sparse_index.indptr()[i]->data()->size();
    }
    for (int i = 0; i < ndim; ++i) {
      out_indices_length += resulted_sparse_index.indices()[i]->data()->size();
    }
